...
pub async fn build_manifest(
    ...
    kubo_url: &str,
    mut runtime_config: BTreeMap<String, serde_json::Value>,
    active_lang: &str,
    old_root_cid: Option<&str>,
) -> Result<BootstrapResult> {
    ...
    // 4. Use compiled-in i18n CIDs (generated by `make src/i18n.json`).
    ...
    if i18n.contains_key(active_lang) {
        runtime_config.insert(
            "i18n".to_string(),
            serde_json::Value::String(active_lang.to_string()),
        );
    }
